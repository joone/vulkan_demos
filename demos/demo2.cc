...
int main(int argc, char** argv) {
  ...
  surface->Initialize(&device_queue_,
  ...
  for (uint32_t i = 0; i < image_count; ++i ) {
    // A framebuffer is a set of textures(attachments) we are rendering into using
    // vkCreateFrameBufer
    // The framebuffer specifies what images are used as attachmenets on which the
    // render pass operates.9
    ...
    vkCmdBeginRenderPass(*surface->GetSwapChain()->GetCommandBuffer(i), &render_pass_begin_info,
