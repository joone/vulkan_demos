...
TEST_F(BasicVulkanTest, BasicVulkanSurface) {
  GetDeviceQueue()->Initialize(VulkanDeviceQueue::GRAPHICS_QUEUE_FLAG |
  ...
  if( vkGetSwapchainImagesKHR(device, surface->GetSwapChain()->handle(),
      &image_count, &swap_chain_images[0]) != VK_SUCCESS ) {
  ...
  for( uint32_t i = 0; i < image_count; ++i ) {
    ...
    vkBeginCommandBuffer(*surface->GetSwapChain()->GetCommandBuffer(i), &cmd_buffer_begin_info );
    vkCmdPipelineBarrier(*surface->GetSwapChain()->GetCommandBuffer(i), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 
        0, 0, nullptr, 0, nullptr, 1, &barrier_from_present_to_clear );
    vkCmdClearColorImage(*surface->GetSwapChain()->GetCommandBuffer(i), swap_chain_images[i], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        &clear_color, 1, &image_subresource_range );
    vkCmdPipelineBarrier(*surface->GetSwapChain()->GetCommandBuffer(i), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
        0, 0, nullptr, 0, nullptr, 1, &barrier_from_clear_to_present );
    if( vkEndCommandBuffer(*surface->GetSwapChain()->GetCommandBuffer(i)) != VK_SUCCESS ) {
...
TEST_F(BasicVulkanTest, BasicVulkanSurface2) {
  ...
  EXPECT_TRUE(render_pass.CreatePipeline(kVertexShaderSource, kFragShaderSource));
  ...
  for( size_t i = 0; i < image_count; ++i ) {
    ...
    vkBeginCommandBuffer(*surface->GetSwapChain()->GetCommandBuffer(i), &graphics_commandd_buffer_begin_info );
    ...
    vkCmdBeginRenderPass(*surface->GetSwapChain()->GetCommandBuffer(i), &render_pass_begin_info, VK_SUBPASS_CONTENTS_INLINE );

    vkCmdBindPipeline(*surface->GetSwapChain()->GetCommandBuffer(i), VK_PIPELINE_BIND_POINT_GRAPHICS,
...
TEST_F(BasicVulkanTest, BasicVulkanSurface3) {
  ...
  EXPECT_TRUE(render_pass.CreatePipeline(kVertexShaderSource, kFragShaderSource, true));
  ...
  while( loop ) {
    if (XPending(gfx::GetXDisplay())) {
      ...
      vkBeginCommandBuffer(*surface->GetSwapChain()->GetCommandBuffer(resource_index), &command_buffer_begin_info);
      ...
      if (GetDeviceQueue()->GetPresentQueue() != GetDeviceQueue()->GetGraphicsQueue()) {
        VkImageMemoryBarrier barrier_from_present_to_draw = {
          ...
          VK_IMAGE_LAYOUT_UNDEFINED,                        // VkImageLayout                          oldLayout
          VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,                  // VkImageLayout                          newLayout
          GetDeviceQueue()->GetPresentQueueFamilyIndex(),   // uint32_t                               srcQueueFamilyIndex
          GetDeviceQueue()->GetGraphicsQueueFamilyIndex(),  // uint32_t                               dstQueueFamilyIndex
      ...
      vkCmdBindVertexBuffers(*surface->GetSwapChain()->GetCommandBuffer(resource_index), 0, 1, vertexBuffer.handle(), &offset );
      ...
      if (GetDeviceQueue()->GetGraphicsQueue() != GetDeviceQueue()->GetPresentQueue()) {
