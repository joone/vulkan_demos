...
*/
  ...
  XID window = XCreateSimpleWindow(display,
    DefaultRootWindow(display), bounds.x(), bounds.y(), bounds.width(), bounds.height(),
    1,
    BlackPixel(display,  default_screen ),
    WhitePixel(display, default_screen ) );

  // XSync( Parameters.DisplayPtr, false );
  XSetStandardProperties(display, window,  "Vulkan Test", "Vulkan Test", None, nullptr, 0, nullptr );
