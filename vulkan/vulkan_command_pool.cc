...
VulkanCommandPool::VulkanCommandPool(VulkanDeviceQueue* device_queue, VulkanSwapChain* swap_chain)
...
// Execute commands on a device we submit them to queues through command buffers.
...
bool VulkanCommandPool::Submit(VkCommandBuffer* command_buffer, uint32_t num_wait_semaphores,
  ...
  if( vkQueueSubmit(device_queue_->GetPresentQueue(), 1, &submit_info, VK_NULL_HANDLE ) != VK_SUCCESS ) {
    return false;
  }
