...
// Render pass describes the internal organization of rendering resources (images/attachments),
// how they are used, and how they change during the rendering process. 
...
bool VulkanRenderPass::CreateFrameBuffer(const VulkanSwapChain* swap_chain, uint32_t resource_index) {
  VkDevice device = device_queue_->GetVulkanDevice();
  ...
  if (vkCreateFramebuffer(device, &framebuffer_create_info, nullptr, &frame_buffers_[resource_index]) != VK_SUCCESS ) {
...
bool VulkanRenderPass::CreatePipeline(const std::string& kVertexShaderSource, 
  ...
  if (resize) {
      ...
      attribute_size = static_cast<uint32_t>(vertex_attribute_descriptions.size());
  }

  VkPipelineVertexInputStateCreateInfo vertex_input_state_create_info = {
    VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, //StructureType  sType
    nullptr,                  // const void   *pNext
    0,                        // VkPipelineVertexInputStateCreateFlags          flags;
    binding_size,  // uint32_t vertexBindingDescriptionCount
    pVertexBindingDescriptions,  // const VkVertexInputBindingDescription *pVertexBindingDescriptions
  ...
  VkGraphicsPipelineCreateInfo pipeline_create_info = {
    ...
    pDynamicStateCreateInfo,      //   (tutorial4)       // const VkPipelineDynamicStateCreateInfo        *pDynamicState
  ...
  if( vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_create_info, nullptr,
