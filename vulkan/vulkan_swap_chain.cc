...
gfx::SwapResult VulkanSwapChain::SwapBuffers() {
  ...
  VkResult result = vkAcquireNextImageKHR(device, swap_chain_,
      UINT64_MAX, current_image_data->render_semaphore, VK_NULL_HANDLE, &image_index );
  ...
  if (GetCurrentCommandPool()->Submit(&current_image_data->command_buffer, 1,
      ...
      &current_image_data->present_semaphore) != VK_SUCCESS) {
    return gfx::SwapResult::SWAP_FAILED;
  }
  ...
  VkPresentInfoKHR present_info = {
    ...
    &current_image_data->present_semaphore, // const VkSemaphore          *pWaitSemaphores
...
bool VulkanSwapChain::InitializeSwapChain(
  ...
  if((vkGetPhysicalDeviceSurfacePresentModesKHR(device_queue_->GetVulkanPhysicalDevice(),
      surface, &present_modes_count, nullptr ) != VK_SUCCESS) ||
  ...
  if( vkGetPhysicalDeviceSurfacePresentModesKHR(device_queue_->GetVulkanPhysicalDevice(),
      surface, &present_modes_count, &present_modes[0] ) != VK_SUCCESS ) {
    std::cout << "Error occurred during presentation surface present modes enumeration!" << std::endl;
    return false;
  }
  ...
  if( vkCreateSwapchainKHR(device, &swap_chain_create_info, nullptr, &swap_chain_) != VK_SUCCESS ) {
  ...
  if( (vkGetSwapchainImagesKHR(device, swap_chain_, &image_count, nullptr ) != VK_SUCCESS) ||
      (image_count == 0) ) {
  ...
  if( vkGetSwapchainImagesKHR(device, swap_chain_, &image_count, &images[0] ) != VK_SUCCESS ) {
    std::cout << "Could not get swap chain images!" << std::endl;
    return false;
  }
...
uint32_t VulkanSwapChain::GetSwapChainNumImages(const VkSurfaceCapabilitiesKHR &surface_capabilities ) {
...
VkSurfaceFormatKHR VulkanSwapChain::GetSwapChainFormat(const std::vector<VkSurfaceFormatKHR> &surface_formats ) {
...
VkExtent2D VulkanSwapChain::GetSwapChainExtent(const VkSurfaceCapabilitiesKHR &surface_capabilities ) {
...
VkImageUsageFlags VulkanSwapChain::GetSwapChainUsageFlags(const VkSurfaceCapabilitiesKHR &surface_capabilities ) {
...
VkSurfaceTransformFlagBitsKHR VulkanSwapChain::GetSwapChainTransform(const VkSurfaceCapabilitiesKHR &surface_capabilities ) {
...
VkPresentModeKHR VulkanSwapChain::GetSwapChainPresentMode(std::vector<VkPresentModeKHR> &present_modes ) {
...
bool VulkanSwapChain::InitializeSwapImages(
  ...
  for (uint32_t i = 0; i < image_count_; ++i) {
    ...
    if( vkCreateFence(device, &fence_create_info, nullptr, &image_data->Fence) != VK_SUCCESS ) {
...
bool VulkanSwapChain::WaitFences(uint32_t* resource_index, uint32_t* image_index) {
    ...
    *resource_index = (*resource_index + 1) % image_count_;
    if(vkWaitForFences(device, 1, &image_data->Fence, VK_FALSE, 1000000000 ) != VK_SUCCESS ) {
...
// for tutorial4
bool VulkanSwapChain::SwapBuffer2(uint32_t resource_index, uint32_t* image_index) {
  printf("VulkanSwapChain::%s\n", __func__);
  std::unique_ptr<ImageData>& image_data = images_[resource_index];
  VkQueue queue = device_queue_->GetPresentQueue();
